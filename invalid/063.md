...
# Loan can be liquidated with zero Amount in `liquidateDefaultedLoanWithIncentive()`
...
```
The value of minAmountDifference could be negative value of amountDue. Then liquidator transfer 0 token to lender.
#### POC
- 1, A borrower borrows 1e18 WETH
- 2, after the default time plus (86400 + 10000) seconds, the liquidator can liquidate the defaulted loan with 0 WETH
```javascript
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    function initialize_group_contract() public {
      ...
      function test_liquidateDefaultedLoanWithIncentive_WithZeroToken() public {
...
```
Create a new file named LenderCommitmentGroup_Smart_WithZeroToken.sol with code above in the fold `./packages/contracts/tests/LenderCommitmentForwarder/extensions/SmartCommitments`, Then run `forge test --mt test_liquidateDefaultedLoanWithIncentive_WithZeroToken -vvv` in terminal. you will get:
```bash
[PASS] test_liquidateDefaultedLoanWithIncentive_WithZeroToken() (gas: 2360720)
Logs:
  principal is  1000000000000000000
  interest is  58155758498224

Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 7.46ms (1.96ms CPU time)
```
## Impact
This resulted in losses for the lender.
## Code Snippet
...
## Recommendation
Loan should not be liquidated with zero Amount