...
# `sharesExchangeRate()` may be zero causing division by zero error in `lenderCommitmentGroupSmart.sharesExchangeRateInverse()`

## Summary
`sharesExchangeRate()` may be zero causing division by zero error in `lenderCommitmentGroupSmart.sharesExchangeRateInverse()`
## Vulnerability Detail
`sharesExchangeRate()` is be divided in `sharesExchangeRateInverse`.
```javascript
...
```
If `sharesExchangeRate()` is 0, causing division by zero error.
...
- 2 causing division by zero error in `lenderCommitmentGroupSmart.sharesExchangeRateInverse()` 
```javascript
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    function test_liquidateDefaultedLoanWithIncentive_DivisionByZeroError() public {
      ...
      uint256 bidId =  smartCommitmentForwarder.acceptCommitmentWithRecipient(
        ...
        lenderCommitmentGroupSmart.sharesExchangeRateInverse();
...
```
Create a new file named LenderCommitmentGroup_Smart_DivisionByZeroError.sol with code above in the fold `./packages/contracts/tests/LenderCommitmentForwarder/extensions/SmartCommitments`, Then run `forge test --mt test_liquidateDefaultedLoanWithIncentive_DivisionByZeroError  -vv` in terminal. you will get:

```bash
Ran 1 test for tests/LenderCommitmentForwarder/extensions/SmartCommitments/LenderCommitmentGroup_Smart_9DivisionByZeroError.sol:LenderCommitmentGroup_Smart_Real_Test
[FAIL. Reason: panic: division or modulo by zero (0x12)] test_liquidateDefaultedLoanWithIncentive_DivisionByZeroError() (gas: 2555114)
Suite result: FAILED. 0 passed; 1 failed; 0 skipped; finished in 7.65ms (1.90ms CPU time)

Ran 1 test suite in 176.40ms (7.65ms CPU time): 0 tests passed, 1 failed, 0 skipped (1 total tests)

Failing tests:
Encountered 1 failing test in tests/LenderCommitmentForwarder/extensions/SmartCommitments/LenderCommitmentGroup_Smart_9DivisionByZeroError.sol:LenderCommitmentGroup_Smart_Real_Test
[FAIL. Reason: panic: division or modulo by zero (0x12)] test_liquidateDefaultedLoanWithIncentive_DivisionByZeroError() (gas: 2555114)
...
## Impact
`sharesExchangeRate()` may be zero causing division by zero error in `lenderCommitmentGroupSmart.sharesExchangeRateInverse()`. Then share token can not be burn for calling  sharesExchangeRateInverse()

## Code Snippet
https://github.com/sherlock-audit/2024-04-teller-finance/blob/defe55469a2576735af67483acf31d623e13592d/teller-protocol-v2-audit-2024/packages/contracts/contracts/LenderCommitmentForwarder/extensions/LenderCommitmentGroup/LenderCommitmentGroup_Smart.sol#L277
## Tool used
...
```diff
    function sharesExchangeRateInverse()
        ...
        returns (uint256 rate_)
    {
+       if(sharesExchangeRate()==0) return 0;
        return
            (EXCHANGE_RATE_EXPANSION_FACTOR * EXCHANGE_RATE_EXPANSION_FACTOR) /
            sharesExchangeRate();
    }
