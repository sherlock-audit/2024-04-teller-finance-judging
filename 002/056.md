...
```
The issue is the use of incorrect parameters for the `uint256 amountDue = getAmountOwedForBid(_bidId, false)`, the second parameter should be true instead of false. This issue resulted in less amount being repaid than should have been.This resulted in losses for the lender.
#### POC
1、A borrower borrows 1e18 WETH
2、after the default time plus 86400 seconds, the liquidator can liquidate the defaulted loan only with 1e18 WETH without any interest
```javascript
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    function initialize_group_contract() public {
      ...
      function test_liquidateDefaultedLoanWithIncentive_missing_interest() public {
      ...
      uint256 bidId =  smartCommitmentForwarder.acceptCommitmentWithRecipient(
        ...
        console.log("principal is ", due.principal);
...
```
Create a new file named LenderCommitmentGroup_Smart_MissingInterest.sol with code above in the fold `./packages/contracts/tests/LenderCommitmentForwarder/extensions/SmartCommitments`, Then run `forge test --mt test_liquidateDefaultedLoanWithIncentive_missing_interest  -vvv` in terminal. you will get:
```bash
[PASS] test_liquidateDefaultedLoanWithIncentive_missing_interest() (gas: 2367885)
Logs:
  principal is  1000000000000000000
  interest is  54984779299847

Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 8.95ms (2.33ms CPU time)
```

## Impact
This issue resulted in less amount being repaid than should have been.This resulted in losses for the lender.
## Code Snippet
