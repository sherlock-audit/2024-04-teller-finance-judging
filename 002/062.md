...
# The parameter of `liquidateDefaultedLoanWithIncentive()` may let liquidator to pay more funds than should.
...
```
The actual value should be minAmountDifference instead of _tokenAmountDifference.
#### POC
- 1, A borrower borrows 1e18 WETH
- 2, after the default time plus 86400 seconds, the liquidator can liquidate the defaulted loan with 10e18 WETH, which 9e18 WETH is an overpayment. 
```javascript
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    function initialize_group_contract() public {
      ...
      function test_liquidateDefaultedLoanWithIncentive_LiquidatorPayMoreFunds() public {
      ...
      uint256 bidId =  smartCommitmentForwarder.acceptCommitmentWithRecipient(
        ...
        console.log("principal owned is ", due.principal);
        console.log("interest is ", due.interest);
       
        //Preparing for liquidation
        principalToken.transfer(address(liquidator), 10e18);
        vm.startPrank(address(liquidator));
        principalToken.approve(address(lenderCommitmentGroupSmart), 10e18);
        //the 9e18 weth is overpayment
        lenderCommitmentGroupSmart.liquidateDefaultedLoanWithIncentive(bidId, 9e18);
        vm.stopPrank();
        
        assertEq(principalToken.balanceOf(address(lenderCommitmentGroupSmart)), 10e18);
...
```
Create a new file named LenderCommitmentGroup_Smart_LiquidatorPayMoreFunds.sol with code above in the fold `./packages/contracts/tests/LenderCommitmentForwarder/extensions/SmartCommitments`, Then run `forge test --mt test_liquidateDefaultedLoanWithIncentive_LiquidatorPayMoreFunds -vvv` in terminal. you will get:
```bash
[PASS] test_liquidateDefaultedLoanWithIncentive_LiquidatorPayMoreFunds() (gas: 2387802)
Logs:
  principal owned is  1000000000000000000
  interest is  54984779299847

Suite result: ok. 1 passed; 0 failed; 0 skipped; finished in 8.55ms (2.25ms CPU time)
```
## Impact
Causing liquidator loss money.
## Code Snippet
...
## Recommendation
using minAmountDifference instead of _tokenAmountDifference
